...
class GameListAdapter(private val gameList: List<ResItem>, private val viewModel: GameListViewModel) :
    ...
    inner class ViewHolder(itemGameListBinding: ItemResBinding) : RecyclerView.ViewHolder(itemGameListBinding.root) {
        val itemGameList = itemGameListBinding.root
        val linearLayout = itemGameListBinding.LinearLayout
        val gameListImage = itemGameListBinding.image
        val blurImage = itemGameListBinding.blurImage
        val gameListName = itemGameListBinding.name
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        ...
        val itemGameListBinding = ItemResBinding.inflate(LayoutInflater.from(mContext), parent, false)
        ...
        holder.linearLayout.setOnClickListener {
            ...
            val adapter = mContext?.let {
                GameDownloadAdapter(
                    it,
                    viewModel.getGameData(gameList.tag)
                )
            }
