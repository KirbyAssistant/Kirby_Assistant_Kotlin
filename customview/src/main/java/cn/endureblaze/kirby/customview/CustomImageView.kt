...
class CustomImageView @JvmOverloads constructor(
            ...
            .forEach {
                when (it) {
                    ...
                    R.styleable.CustomImageView_circle_dot_radius ->
                        mCircleDotRadius = array.getDimension(
                            it,
    ...
    private fun buildRoundPath() {//构建圆角类型的Path路径
        if (!mShowBorder) {//绘制不带边框的圆角实际上只需要把一个圆角矩形扔进path即可
            ...
            floatArrayOf(
                mLeftTopRadiusX - mBorderWidth / 2.0f, mLeftTopRadiusY - mBorderWidth / 2.0f,
                ...
                mLeftBottomRadiusX - mBorderWidth / 2.0f, mLeftBottomRadiusY - mBorderWidth / 2.0f
            ).run {
    ...
    private fun drawCircleDot(canvas: Canvas?) {
        canvas?.run {
            drawCircle(
                (mRadius + mRadius * (sqrt(2.0) / 2.0f)).toFloat(),
                (mRadius - mRadius * (sqrt(2.0) / 2.0f)).toFloat(),
                mCircleDotRadius,
                mCircleDotPaint
            )
